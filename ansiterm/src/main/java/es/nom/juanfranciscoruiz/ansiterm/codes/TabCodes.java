 ...
 *
 * The following constants and functions  allow an application to set the tab stop locations within the console window,
 * remove them, and navigate between them.
...
public class TabCodes {
    ...
    /**
     * Returns an instance of TabCodes
     * @return an instance of TabCodes
     */
    public TabCodes getInstance(){
        if (this.tabCodes == null){
            this.tabCodes = new TabCodes();
        }
        return tabCodes;
    }
    /**
     ...
     * Behavior.......: Advance the cursor to the next column (in the same row) with a tab stop
     * Notes..........: For both CHT and CBT, <n> is an optional parameter that (default=1) indicating how many times to
     * advance the cursor in the specified direction. If there are no tab stops set via HTS, CHT and CBT will treat the
     * first and last columns of the window as the only two tab stops. Using HTS to set a tab stop will also cause the
     * console to navigate to the next tab stop on the output of a TAB (0x09, ‘\t’) character, in the same manner as CHT.
     *
     */
    public String set_TAB_CURSOR_HORIZONTAL_TAB(int n){return ESC + "[" + n + "I";};
     ...
     * Behavior.......: Move the cursor to the previous column (in the same row) with a tab stop
     * @param n number of times to move the cursor
     */
    public  String set_TAB_CURSOR_BACKWARDS_TAB(int n){ return ESC + "[" + n + "Z";};
     ...
     * Behavior.......: Clears the tab stop in the current column, if there is one. Otherwise does nothing.
     */
     ...
     *
     * Sequence.......: ESC [3g
     * Description....: Removes all configured tab stops, effectively resetting the tab stop settings.
