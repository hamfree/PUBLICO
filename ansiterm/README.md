...
- Seguir con la migracion del retorno de secuencias ASCII a las clases del paquete `es.juanfranciscoruiz.ansiterm.codes` y hacer que AnsiTerm las utilize con `System.out.print()`.
    - Solo faltan dos grupos por definir: `Soft Reset` y luego Enums para los códigos de las `Input Sequences` o secuencias de entrada, que son tres:
        - Cursor Keys (Teclas del cursor)
        - Numpad and Function Keys (Teclas del teclado numérico y teclas de Función)
        - Modifiers (Las Teclas Ctrl, Alt y Shift, que actúan como modificadoras de teclas)
          Por las secuencias de entrada de las teclas del cursor (Cursor Keys) existen dos modos: `Normal Mode` y `Application Mode`, que habrá que completar.
          Los Modifiers o modificadores son las teclas *Ctrl*, *Alt* y *Shift*, teniendo en cuenta que la tecla de Ctrl _izquierda_ y la tecla Alt _derecha_  cuando se
          pulsan juntas se tratan como la tecla *AltGr*.
- Implementar demostraciones para los nuevos codigos de escape ASCII en *CharacterSetModeCodes*, *DrawChars*, *TabCodes*, *ViewportPositioningCodes*, etc..
- Completar la clase de pruebas unitarias para la clase *ANSITerm* (y discurrir cómo hacerlo, porque ANSITerm actúa sobre el terminal...).
- Refactorizar las clases de demostración en `es.juanfranciscoruiz.ansiterm.app` como lo esta *ScreenAndLineDeletions*, que usa la clase de utilidad *Util*.
- ¿Generación de widgets o controles visuales al estilo de la utilidad Linux `dialog`? Evaluarlo, quizá implementar los widgets en una nueva librería, que se apoye en esta.
