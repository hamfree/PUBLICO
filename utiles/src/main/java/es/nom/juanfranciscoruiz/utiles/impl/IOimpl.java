...
public class IOimpl implements IO {
     ...
     */
    private static UnclosableInputStreamDecorator inputStream;
    ...
    /**
     * Retrieves the current instance of the UnclosableInputStreamDecorator object
     * associated with this class. This stream is designed to prevent being closed,
     * while still providing all standard InputStream functionalities.
     *
     * @return the UnclosableInputStreamDecorator instance that wraps an InputStream.
     */
    public static UnclosableInputStreamDecorator getInputStream() {
        return inputStream;
    }

    /**
     * Sets the static input stream for the IO class to the specified
     * UnclosableInputStreamDecorator. This allows configuring a
     * custom InputStream wrapper that ensures the original stream
     * cannot be closed.
     *
     * @param inputStream the UnclosableInputStreamDecorator instance
    ...
    public static void prtln(int ls, Object... args) throws IllegalArgumentException {
        ...
        if (ls > 0) {
            sb.append(String.valueOf(LS).repeat(ls));
        }
    ...
    public static String repeatCharacter(Character character, int times) {
        ...
        if (character != null && times > 0) {
            sb.append(String.valueOf(character).repeat(times));
        } else {
            if (LOGGER.isErrorEnabled()) {
                LOGGER.error(ERR_PARAM);
            }
